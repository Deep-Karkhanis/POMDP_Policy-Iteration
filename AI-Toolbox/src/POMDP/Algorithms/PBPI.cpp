...
namespace AIToolbox::POMDP {
    PBPI::PBPI(const size_t nBeliefs, const unsigned h, const double t, const double l1disteps, AIToolbox::POMDP::Belief b) :
            beliefSize_(nBeliefs), horizon_(h)
    {
        setTolerance(t);
        setL1DistEps(l1disteps);
        bel_ = b;
    }

    void PBPI::setHorizon(const unsigned h) {
        horizon_ = h;
    }
    void PBPI::setTolerance(const double t) {
        if ( t < 0.0 ) throw std::invalid_argument("Tolerance must be >= 0");
        tolerance_ = t;
    }

    void PBPI::setBeliefSize(const size_t nBeliefs) {
        beliefSize_ = nBeliefs;
    }

    void PBPI::setL1DistEps(const size_t eps) {
        l1disteps_ = eps;
    }

    unsigned PBPI::getHorizon() const { return horizon_; }
    double PBPI::getTolerance() const { return tolerance_; }
    size_t PBPI::getBeliefSize() const { return beliefSize_; }
    size_t PBPI::getL1DistEps() const { return l1disteps_; }

    VList PBPI::crossSum(const VList & l1, const VList & l2, const size_t a, const bool order) {
        VList c;
    ...
    void PBPI::updateFSC(const VList &w)
    {
        if(w.size() == 0)
            return;
        if(howard_ == 1)
        ...
        for(size_t alpDInd=0;alpDInd<(w.size()); alpDInd++)
        {
            ...
            for(size_t alpInd=0;alpInd<(cpyFsc.nodes.size()); alpInd++)
            {
                  ...
                  )
                {
                    ...
                    for(int ss=0;ss<(alp.values.rows());ss++)
                    {
                        ...
                        {
                            // std::cout<<"How can alpD and alp have diff values???\n";
                            dontAdd=0;
                            break;
                        }
                    }
                    if(dontAdd == 1)
                    {
                        ...
                        // std::cout<<"tf: "<<alpDInd<<" same as prev\n";
                        break;
                    }
                }
            }
            ...
            for(size_t alpInd=0;alpInd<(cpyFsc.nodes.size()); alpInd++)
            {
                ...
                if(doms==1)
                {
                    ...
                    fsc_.nodes[alpInd].mark=1;
                    dontAdd = 1;
                    // std::cout<<"tf: "<<alpDInd<<" doms\n";
                    // for(int ss=0;ss<(alp.values.rows());ss++)
                    // {
                    //     std::cout<<alpD.values(ss, 0)<<" > "<<alp.values(ss, 0)<<"\n";
                    // }
                    break;
                }
            }
            ...
            fsc_.maxId++;
            // std::cout<<"tf: "<<alpDInd<< " node added\n";
        ...
        if(howard_ == 1)
        {
            ...
            {
                for(std::vector<MchnState>::iterator j=fsc_.nodes.end();
                            j-- != fsc_.nodes.begin();)
                    {
                        if((j->mark) != 2)
                        {
                            // std::cout<<"Pruning Node "<<(j-fsc_.nodes.begin())<<": "<<j->action;
                            // for(int iter=0; iter<O; iter++) std::cout<<j->obsSucc[iter]<<" ";
                            // std::cout<<"\n";
                            fsc_.nodes.erase(j);
                            break;
                        }
