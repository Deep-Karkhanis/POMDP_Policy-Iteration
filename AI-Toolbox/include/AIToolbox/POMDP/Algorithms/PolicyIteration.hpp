...
namespace AIToolbox::POMDP {
    ...
    class PolicyIteration {
        ...
        private:
            ...
            void updateFSC(const VList &w, int timestep);
            void makeInitialFSC();
    ...
    std::tuple<double, ValueFunction> PolicyIteration::operator()(const M & model) {
        ...
        while ( timestep<horizon_ && (fsc_.nodes.size() <= nodeLimit_) && ( !useTolerance || variation > tolerance_ ) )
        {
            ...
            std::cout<<" "<<maxNode<<" "<<fsc_.nodes[maxNode].action<<" ";
            for(int valI=0;valI<(int)S;valI++)
                std::cout<<fsc_.nodes[maxNode].values[valI]<<" ";
            for(int valI=0;valI<(int)O;valI++)
                std::cout<<fsc_.nodes[maxNode].obsSucc[valI]<<" ";
            std::cout<<std::endl;
            ...
            if(howard_==0 && IMthd_==1)
            {
                ...
                updateFSC(w1, timestep);
            }
            else
            {
                updateFSC(w, timestep);
                if(timestep == 5 || timestep == 6){
                    std::cout<<"*****************************************\n";
                    std::cout<<"FSC: ";
                    for(int mchSt=0;mchSt<(int)fsc_.nodes.size();mchSt++)
                    {
