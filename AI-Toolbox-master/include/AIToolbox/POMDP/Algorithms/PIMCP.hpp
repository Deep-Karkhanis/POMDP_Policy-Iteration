#ifndef AI_TOOLBOX_POMDP_PIMCP_HEADER_FILE
#define AI_TOOLBOX_POMDP_PIMCP_HEADER_FILE
...
namespace AIToolbox::POMDP {
    /**
     * @brief This class represents the PIMCP online planner using UCB1.
     *
     ...
     *
     * PIMCP plans for a single belief at a time. It follows the logic of
     ...
     *
     * Since PIMCP expands a tree, it can reuse work it has done if
     ...
     * action/observation pair, which can be expensive, PIMCP uses particle
    ...
    class PIMCP {
        ...
        public:
             ...
             *
             * @param m The POMDP model that PIMCP will operate upon.
             * @param beliefSize The size of the initial particle belief.
             * @param iterations The number of episodes to run before completion.
             * @param exp The exploration constant. This parameter is VERY important to determine the final PIMCP performance.
             */
            PIMCP(const M& m, size_t beliefSize, unsigned iterations, double exp, ValueFunction fsc_inp);
            ...
            /**
             * @brief This function sets the number of performed rollouts in PIMCP.
             *
            ...
            /**
             * @brief This function sets the new exploration constant for PIMCP.
             *
             * This parameter is EXTREMELY important to determine PIMCP
        ...
        private:
             ...
             * times specified by PIMCP's parameters. While doing so it
             * builds a tree of explored outcomes, from which PIMCP will
            ...
            /**
             * @brief This function implements the rollout policy for PIMCP.
             *
    ...
    template <typename M>
    PIMCP<M>::PIMCP(const M& m, const size_t beliefSize, const unsigned iter, const double exp, ValueFunction fsc_inp) :
            model_(m), S(model_.getS()), A(model_.getA()), beliefSize_(beliefSize),
            iterations_(iter), exploration_(exp), graph_(), rand_(Impl::Seeder::getSeed()) { fsc_ = fsc_inp;}

    template <typename M>
    size_t PIMCP<M>::sampleAction(const Belief& b, const unsigned horizon) {
        // Reset graph
        graph_ = BeliefNode(A);
    ...
    template <typename M>
    size_t PIMCP<M>::sampleAction(const size_t a, const size_t o, const unsigned horizon) {
        ...
        if ( ! graph_.belief.size() ) {
            AI_LOGGER(AI_SEVERITY_WARNING, "PIMCP lost track of the belief, restarting with uniform..");
            auto b = Belief(S); b.fill(1.0/S);
            return sampleAction(b, horizon);
        }
    ...
    template <typename M>
    size_t PIMCP<M>::runSimulation(const unsigned horizon) {
        if ( !horizon ) return 0;
    ...
    template <typename M>
    double PIMCP<M>::simulate(BeliefNode & b, const size_t s, const unsigned depth) {
        b.N++;
    ...
    // template <typename M>
    // double PIMCP<M>::rollout(size_t s, unsigned depth) {
    ...
    double PIMCP<M>::rollout(size_t s, unsigned depth) {
        ...
        std::uniform_int_distribution<size_t> coinT(0, 1);
        if (coinT(rand_) == 0){
            std::uniform_int_distribution<size_t> generator(0, A-1);
            for ( ; depth < maxDepth_; ++depth ) {
                std::tie( s, rew ) = model_.sampleSR( s, generator(rand_) );
            ...
            double maxExp=fsc_[0][0].values[s];
            for(int alpInd=0;alpInd<(int)(fsc_[0].size());alpInd++)
            {
                double val = (fsc_[0][alpInd].values[s]); 
                if(maxExp < val)
                    maxExp = val;
            }
            return maxExp;
    ...
    template <typename M>
    template <typename Iterator>
    Iterator PIMCP<M>::findBestA(Iterator begin, Iterator end) {
        return std::max_element(begin, end, [](const ActionNode & lhs, const ActionNode & rhs){ return lhs.V < rhs.V; });
    }

    template <typename M>
    template <typename Iterator>
    Iterator PIMCP<M>::findBestBonusA(Iterator begin, Iterator end, const unsigned count) {
        ...
        // substituted with something else to produce different PIMCP variants.
    ...
    template <typename M>
    typename PIMCP<M>::SampleBelief PIMCP<M>::makeSampledBelief(const Belief & b) {
        SampleBelief belief;
    ...
    template <typename M>
    void PIMCP<M>::setBeliefSize(const size_t beliefSize) {
        beliefSize_ = beliefSize;
    }

    template <typename M>
    void PIMCP<M>::setIterations(const unsigned iter) {
        iterations_ = iter;
    }

    template <typename M>
    void PIMCP<M>::setExploration(const double exp) {
        exploration_ = exp;
    }

    template <typename M>
    const M& PIMCP<M>::getModel() const {
        return model_;
    }

    template <typename M>
    const typename PIMCP<M>::BeliefNode& PIMCP<M>::getGraph() const {
        return graph_;
    }

    template <typename M>
    size_t PIMCP<M>::getBeliefSize() const {
        return beliefSize_;
    }

    template <typename M>
    unsigned PIMCP<M>::getIterations() const {
        return iterations_;
    }

    template <typename M>
    double PIMCP<M>::getExploration() const {
        return exploration_;
    }
}
